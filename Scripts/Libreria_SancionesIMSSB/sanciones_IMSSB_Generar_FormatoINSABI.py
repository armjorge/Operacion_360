...
def A_confirm_files_exists(folder_path): 
    folder_name = os.path.basename(folder_path)
    print(f"Looking for the folder...{folder_name}")
...
def B_carga_desde_el_GOOGLESHEET(sheets_in_management, credentials_file):
...
def C_generating_single_dataframe(df_INSABI_gsheet, df_audit, df_relacion, folder_name):
        # Renombrar columnas para el C_generating
    #change_columns = {"FOLIO FISCAL NOTA DE CREDITO": "XML_UUID_NC", "NOTA DE CREDITO (FOLIO Y SERIE)":"Filename", "MONTO SANCIONADO": "XML_Total" }
    #df_audit= df_audit.rename(columns = change_columns) 
    print("Step C generar un sólo data frame, revisando columnas necesarias")
...
def D_convert_formato_INSABI_toPDF(excel_file, orientation, pdf_file):
    """
    Convert an Excel file to a PDF with specified orientation (landscape or portrait).
...
def F_estatusarmado(folder_name):
    ...
    pdf_file = os.path.join(folder_name, f"{file_name}_formato_INSABI.pdf")
        
    Cedula = os.path.join(folder_name, f"{file_name}.pdf")
    Formato_excel_INSABI = os.path.join(folder_name, f"{file_name}_formato_INSABI.pdf")
    Carta = os.path.join(folder_name, f"{file_name}_carta.pdf")
    Portada = os.path.join(folder_name, f"{file_name}_portada.pdf")
    ...
    print("\nGeneración Revisión y fusión finalizada, por favor genera ZIP e  y PDF de impresión")
...
def GenerarFormatoINSABI(folder_name, credentials_file, template_file):
    df_audit, df_relacion = A_confirm_files_exists(folder_name)
    
    if df_audit is not None and df_relacion is not None:
        # Proceed with further processing
        print("\nStep A listo. df_audit y df_relación listos para el siguiente paso\n")
    else:
        print("File check failed. Please try again.")

    ### Cargar el RAW_INSABI ###

    cargar_desde_GSHEET = {'df_RAW_INSABI': ('RAW_INSABI', 'A:AA')}#,'df_anotherSheet': ('Another_Sheet_Name', 'A:B')}
    dataframes = B_carga_desde_el_GOOGLESHEET(cargar_desde_GSHEET, credentials_file)    
    ...
    print(
        '\nStep B listo. df_INSABI cargado desde el google sheet\n'
    )


    # now get the base name and pass it along
    base = os.path.basename(folder_name)
    output_file_formato_INSABI = os.path.join(folder_name, f"{base}_formato_INSABI.xlsx")
    output_PDF_file_formato_INSABI = os.path.join(folder_name, f"{base}_formato_INSABI.pdf")
    master_df = C_generating_single_dataframe(df_RAW_INSABI, df_audit,df_relacion, base )
     
    if master_df is not None:
        print("\nPaso C completado, es momento de guardar el formato _formato_INSABI\n")
        
        
        # Save `master_df` to the output file
        excel_rewritting = input("Tenemos la información necesaria para generar el archivo, procedemos a guardarlo? yes or not  ").strip().lower()
        if excel_rewritting == "yes":
            master_df.to_excel(output_file_formato_INSABI, index=False)
            print(f"Formato INSABI guardado {os.path.basename(output_file_formato_INSABI)}.")
        else:
    ...
    formato = input("Abrimos el archivo para aplicar formato? (yes or no): ").strip().lower()
    if formato == 'yes':
        # 1) Open the template for formatting
        if os.path.exists(template_file):
            open_excel(template_file)
        else:
            print(f"Template file not found: {template_file}")

        # 2) Open the INSABI output file for formatting
        if os.path.exists(output_file_formato_INSABI):
            open_excel(output_file_formato_INSABI)
        else:
            print(f"Output file not found: {os.path.basename(output_file_formato_INSABI)}")

        # 3) Pause until user is done
        input("Push Enter when you finish applying the format and want to continue: ")
    D_convert_formato_INSABI_toPDF(output_file_formato_INSABI, 'landscape', output_PDF_file_formato_INSABI)    
    #E_generateletter(master_df)
    #G_generatezip(folder_name)
    F_estatusarmado(folder_name)

print("SCRIPT Finalizada")